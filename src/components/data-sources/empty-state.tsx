...
export function EmptyState({ onAddSample } : EmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 className="text-xl font-semibold text-gray-400 mb-4">No data in your product table</h3>
      <p className="text-gray-500 mb-8 max-w-md">
        Your schema is ready, but no data yet. Import data from a CSV/ JSON file or add sample data to test your
        API endpoints.
      </p>
      <div className="flex gap-4">
        ...
        className="flex items-center gap-2 px-6 py-3 bg-secondary text-white rounded-lg font-medium hover:bg-teal-700 transition-colors">
         ...
         <ImportDataModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onImport={handleImport} />
      </div>
    </div>
  )
}
